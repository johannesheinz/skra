// Package export renders address books and contacts to downloadable vCard and
// CSV. It is pure transformation (no database access) so it is easy to test;
// callers gather the data and pass it in.
package export
...
// VCardEntry is one contact for vCard export: its canonical vcard_raw plus,
// optionally, the normalized JPEG photo to embed.
...
// WriteVCards writes the entries as a single vCard stream. Entries without a
// photo are emitted verbatim (preserving stored fidelity); entries with a photo
// are re-encoded to inject a base64 PHOTO so the photo is not double-stored.
...
// injectPhoto parses a single vCard, sets its PHOTO to a base64 JPEG data URI,
// and re-encodes it as vCard 4.0.
