...
// IssueCSRF generates a token, sets it as the CSRF cookie, and returns it so a handler can embed it in a form.
// Uses the double-submit pattern: no server-side storage and no signing key required.
...
// VerifyCSRF checks that the token in the form matches the one in the cookie, in constant time.
// The request's form must already be parsed, or this parses it.
