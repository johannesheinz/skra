...
// Cookie names.
// The session cookie carries the opaque session id; the CSRF cookie carries the double-submit token.
const (
...
// Secure is driven by configuration (the external scheme), not the internal connection.
// SameSite=Lax blunts CSRF on top of the explicit token check.
...
// NewCSRFCookie builds the double-submit CSRF cookie.
// It is short-lived and scoped like the session cookie.
