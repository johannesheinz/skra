...
// BuildCanonicalRaw produces a vCard 4.0 string from structured fields, used when a uid must be re-minted (create-new on collision).
...
// splitCards extracts each BEGIN:VCARD…END:VCARD block as its own string so the blocks can be decoded independently for per-card error isolation.
