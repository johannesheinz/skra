...
// vacuumIntoSQL builds a VACUUM INTO statement with a safely single-quoted path (the path is not user input, but quoting keeps it robust).
...
// isFreshDatabase reports whether path refers to a not-yet-initialized database (absent or zero-length file), which is when auto_vacuum is established.
