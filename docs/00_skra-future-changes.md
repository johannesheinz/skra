...
- **Preferences:** `prefers-reduced-motion` disables transitions/animations; high contrast (firmer borders, thicker focus ring, full-contrast muted text) is applied both automatically via `prefers-contrast: more` and via a per-user opt-in stored in `UIPreferences.A11y.HighContrast` (a checkbox on the account page that stamps `data-contrast="more"` on `<html>`).
